...
def createLables():
    dirs = os.listdir("Face_Recognition_Script\\"+training_folder)
    for users in dirs:
        lable = int(users[users.find("@")+1:len(users)])
        names[lable] = users[0:users.find("@")]
        subfolders = training_folder + "/" + users
        imagesName = os.listdir("Face_Recognition_Script\\"+subfolders)
        for image in imagesName:
            imagePath = subfolders + "/" + image
            face = cv2.imread("Face_Recognition_Script\\"+imagePath)
